...
/**
 * Servlet implementation class WarehousesServlet2
 */
@WebServlet("/WarehousesServlet2")
public class WarehousesServlet2 extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * Default constructor. 
     */
    public WarehousesServlet2() {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
        	out.println("<title>Warehouses</title>");
        	out.println("<body style='background-color:#D8CEF6;'>");
	        Class.forName("com.mysql.jdbc.Driver");
			
	        String query = request.getParameter("query");
	        out.println("<h1>Warehouses</h1>");
	        ...
	        if (query.length() > 6 && 
		        ...
		        while(queryResult.next()) {
		        	...
		        	out.println("<td>");
		        	out.println("<form action='ProductsServlet2' method='post'>");
		        	out.println("<button name ='query'  value='select * from Products where ID_Warehouses="+queryResult.getObject(1)+"' >SHOW PRODUCTS</button>");
